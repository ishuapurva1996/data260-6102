...
def report_arithmetic(report_dir, computed, summary):
    ...
    for key, value in summary["deployment_choice"].items():
        if key in choice:
            require(choice[key] == value, f"Published deployment {key} differs from campaign summary")
    ...
    if check("campaign_integrity_and_provenance", load):
        ...
        if report_dir is not None:
            if "recomputed" in report:
                check("published_tables_and_deployment_choice", lambda: report_arithmetic(Path(report_dir), report["recomputed"], summary))
            else:

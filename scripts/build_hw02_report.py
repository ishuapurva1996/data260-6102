...
def pages(code_ref, commit):
    ...
    ], code=[lines(graph + "state.py", 8, 28), lines(graph + "nodes.py", 83, 88)], pictures=[panels + "P3-normal.png"], caption="Saved run output: the Planner and Reviewer finish in two turns. This check is separate from the 75 Part 4 measurements."))
    ...
    ], code=[lines(graph + "nodes.py", 126, 153), lines(graph + "nodes.py", 155, 174)], pictures=[panels + "P3-normal-short.png"], caption="Saved run output: the current draft passes validation and receives Reviewer approval."))
    ...
    ], code=[function(graph + "nodes.py", "supervisor_node"), function(graph + "router.py", "router_logic")], pictures=[panels + "P3-controlled-short.png"], caption="Saved controlled-run output: a forced Reviewer issue keeps the loop running until the ten-turn ceiling."))
    ...
    ], code=[function(graph + "workflow.py", "build_graph"), lines("code/agents_graph.py", 194, 203)], pictures=[panels + "P3-controlled.png"], caption="Saved controlled-run output: five Planner-Reviewer pairs end at the ten-turn ceiling."))
    ...
    ], code=[lines(graph + "contracts.py", 15, 24), lines(graph + "contracts.py", 27, 43)], pictures=[panels + "p4q1-schema-valid-output.png"], caption="Saved run output from a real schema trial: validation and Reviewer approval both pass."))
    ...
    ], code=[lines(graph + "nodes.py", 97, 107), lines(graph + "nodes.py", 126, 139)], pictures=[panels + "p4q2-scripted-repair-output.png"], caption="Scripted test output: an invalid Planner response, a valid replacement, then Reviewer approval on turn 3. Excluded from all 75 measured trials."))

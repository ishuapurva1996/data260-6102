...
def git_metadata():
    def git(*args):
        try:
            ...
            raise IntegrityError(f"Git metadata unavailable: {type(exc).__name__}: {exc}") from exc
...
def execute_trial(path, manifest, slot, *, folder="trials"):
    ...
    write_new(directory/"started.json", dict(**base, started_at=utc_now(), command=args, git=git))
...
def run_campaign(path, *, limit=None):
    ...
    with campaign_lock(path):
        ...
        warmups = path/"warmups"
        if warmups.exists():
            try:
                for directory in sorted(warmups.iterdir()):
                    if not directory.is_dir():
                        raise IntegrityError("Unexpected artifact outside a warm-up trial directory")
                    slot = dict(trial_id=directory.name, cohort="warmup", order=0,
                                max_turns=10, input_key="schema", pair=None)
                    warm = read_trial(path, manifest, slot, folder="warmups")
                    if warm is None or warm["status"] not in {"accepted", "turn_limit"}:
                        raise IntegrityError(f"Failed or incomplete excluded warm-up: {directory.name}")
            except (IntegrityError, OSError, KeyError, TypeError, ValueError) as exc:
                raise IntegrityError("Campaign has invalid/error/unknown warm-up evidence; preserve it, "
                                     "diagnose the failure, and prepare a new campaign") from exc
